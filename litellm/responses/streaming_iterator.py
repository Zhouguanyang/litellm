...
class BaseResponsesAPIStreamingIterator:
    ...
    @staticmethod
    def _copy_stream_model(model_obj: Any) -> Any | None:
        if hasattr(model_obj, "model_dump") and hasattr(type(model_obj), "model_validate"):
            try:
    ...
    def _track_completed_output_item(self, stream_event: Any) -> None:
        if getattr(stream_event, "type", None) != ResponsesAPIStreamEvents.OUTPUT_ITEM_DONE:
            return
        ...
        self._completed_output_items_by_index[output_index] = self._dump_stream_output_item(item)
    ...
    def _response_event_for_logging(self, stream_event: Any) -> Any:
        ...
        output_items = [
            self._dump_stream_output_item(item) for _, item in sorted(self._completed_output_items_by_index.items())
        ]
        ...
        try:
            ...
            if isinstance(parsed_chunk, dict):
                ...
                if openai_responses_api_chunk and _chunk_type in (
                    ...
                    self.completed_response = self._response_event_for_logging(openai_responses_api_chunk)
