...
def resolve_images_url(api_base: str | None) -> str:
    configured_base_url: Final = (
        api_base or get_secret_str("OPENROUTER_API_BASE") or DEFAULT_OPENROUTER_API_BASE
    ).rstrip("/")
    base_url: Final = configured_base_url.removesuffix(LEGACY_CHAT_COMPLETIONS_SUFFIX)
...
def map_size_to_aspect_ratio(size: str) -> str | None:
    ...
    """
    match: Final = PIXEL_SIZE_PATTERN.fullmatch(size.strip())
    if match is None:
        return None
    ...
    closest: Final = min(SUPPORTED_ASPECT_RATIOS, key=lambda ratio: abs(ratio[0] / ratio[1] - target))
...
def _set_usage_and_cost(
    ...
    image_tokens: Final = usage.completion_tokens_details.image_tokens if usage.completion_tokens_details else None
    input_image_tokens: Final = (usage.prompt_tokens_details.image_tokens or 0) if usage.prompt_tokens_details else 0
    ...
    if usage.cost is not None:
        additional_headers: Final = {
            **(model_response._hidden_params.get("additional_headers") or {}),
            "llm_provider-x-litellm-response-cost": usage.cost,
        }
