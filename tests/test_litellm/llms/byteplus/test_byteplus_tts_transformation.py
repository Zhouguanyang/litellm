...
class RequestRecorder:
    ...
    def __call__(self, request: httpx.Request) -> httpx.Response:
        ...
        chunk: str = base64.b64encode(b"mocked-audio").decode("utf-8")
        return httpx.Response(
            status_code=200,
            text=f'{{"code":0,"data":"{chunk}"}}\n{{"code":20000000,"message":"ok"}}',
        )
...
class TestBytePlusTextToSpeechConfig:
    ...
    def test_get_complete_url_prefers_request_api_base(self, monkeypatch):
        monkeypatch.setenv("BYTEPLUS_TTS_API_BASE", "https://voice.environment.example/api/v3")
        config = BytePlusTextToSpeechConfig()
        url = config.get_complete_url(
            model="seed-tts-2.0",
            api_base="https://tts-proxy.example/api/v3",
            litellm_params={},
        )
        assert url == "https://tts-proxy.example/api/v3/tts/unidirectional"
    ...
    def test_litellm_speech_preserves_client_and_custom_api_base(self):
        ...
        assert str(recorder.request.url) == "https://tts-proxy.example/api/v3/tts/unidirectional"
        assert response.content == b"mocked-audio"
